...
init_func() {
    ...
    "$PWD"/build/ethermintd genesis gentx $KEY"$i" 1000000000000000000stake --keyring-backend test --home "$DATA_DIR$i" --keyring-backend test --chain-id $CHAINID
...
if [[ -z $TEST || $TEST == "rpc" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        MODE=$MODE HOST=$HOST_RPC go test -count=1 ./tests/e2e/... -timeout=300s -v -short
        MODE=$MODE HOST=$HOST_RPC go test -count=1 ./tests/rpc/... -timeout=300s -v -short
