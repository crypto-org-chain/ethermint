  ...
  outputs = { self, nixpkgs, nix-bundle-exe, gomod2nix, flake-utils }:
    ...
    (flake-utils.lib.eachDefaultSystem
      (system:
        ...
        in
        rec {
          packages = pkgs.ethermint-matrix // {
            inherit (pkgs) rocksdb;
          };
          apps = {
            ethermintd = mkApp packages.ethermintd;
            ethermintd-testnet = mkApp packages.ethermintd-testnet;
          };
          defaultPackage = packages.ethermintd;
          defaultApp = apps.ethermintd;
          devShells = {
      ...
      overlay = final: super: {
        ...
        make-tarball = drv: final.runCommand "tarball-${drv.name}"
          ...
          tar cfv - -C "${drv}" \
            ...
            | gzip -9 > $out
        '';
        bundle-win-exe = drv: final.callPackage ./nix/bundle-win-exe.nix { ethermintd = drv; };
      } // (with final;
        let
          ...
          binaries = builtins.listToAttrs (builtins.map
            ({ network, pkgtype }: {
              ...
              value =
                let
                  ethermintd = callPackage ./. {
                    inherit rev network;
                  };
                  bundle =
                    if stdenv.hostPlatform.isWindows then
                      bundle-win-exe ethermintd
                    else
                      bundle-exe ethermintd;
                in
