...
func (suite *StateTransitionTestSuite) TestOsakaMaxTxGas() {
	...
	suite.Run("enforces governance-configured MaxTxGas instead of geth default", func() {
		...
		evmParams.MaxTxGas = customMaxTxGas
		suite.Require().NoError(suite.App.EvmKeeper.SetParams(suite.Ctx, evmParams))
		suite.App.EvmKeeper.RemoveParamsCache(suite.Ctx)

		customCfg, err := suite.App.EvmKeeper.EVMConfig(suite.Ctx, suite.App.EvmKeeper.ChainID(), common.Hash{})
		suite.Require().NoError(err)
		customCfg.TxConfig = suite.App.EvmKeeper.TxConfig(suite.Ctx, common.Hash{})

		overCap := &core.Message{
			To:              &to,
			From:            suite.Address,
			Nonce:           suite.StateDB().GetNonce(suite.Address),
			Value:           big.NewInt(0),
			GasLimit:        customMaxTxGas + 1,
			GasPrice:        big.NewInt(0),
			GasFeeCap:       big.NewInt(0),
			GasTipCap:       big.NewInt(0),
			SkipNonceChecks: true,
		}
		_, err = suite.App.EvmKeeper.ApplyMessageWithConfig(suite.Ctx, overCap, customCfg, true)
		suite.Require().Error(err, "must reject gasLimit above the governance cap even though it's below geth's default")
		suite.Require().ErrorIs(err, core.ErrGasLimitTooHigh)
		...
		atCap := &core.Message{
			...
			Nonce:           suite.StateDB().GetNonce(suite.Address),
			Value:           big.NewInt(0),
			GasLimit:        customMaxTxGas,
			GasPrice:        big.NewInt(0),
			GasFeeCap:       big.NewInt(0),
			GasTipCap:       big.NewInt(0),
			SkipNonceChecks: true,
		}
		result, err := suite.App.EvmKeeper.ApplyMessageWithConfig(suite.Ctx, atCap, customCfg, true)
		suite.Require().NoError(err)
		suite.Require().False(result.Failed())
	})
}

// TestCheckMaxTxGas verifies the cap only applies once Osaka is active.
func TestCheckMaxTxGas(t *testing.T) {
	const maxTxGas = uint64(1_000_000)

	err := keeper.CheckMaxTxGas(maxTxGas+1, maxTxGas, params.Rules{IsOsaka: true})
	require.ErrorIs(t, err, core.ErrGasLimitTooHigh, "must reject an over-cap gasLimit once Osaka is active")

	err = keeper.CheckMaxTxGas(maxTxGas+1, maxTxGas, params.Rules{IsOsaka: false})
	require.NoError(t, err, "cap must not apply before Osaka activates")
}
