...
* (rpc) [#923](https://github.com/crypto-org-chain/ethermint/pull/923) fix(rpc): include block-gas-exceeded txs in `eth_getBlockReceipts`, compute block-wide eth `cumulativeGasUsed`, and restore indexer-overwrite guard on `GetTransactionReceipt`. **Note:** existing KV indexer data stores per-tx cumulative; multi-eth-tx blocks need reindexing to return accurate block-wide `cumulativeGasUsed` from the O(1) `eth_getTransactionReceipt` path.
