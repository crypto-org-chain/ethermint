...
* (evm) [#939](https://github.com/crypto-org-chain/ethermint/pull/939) fix(evm): don't abort `debug_traceTransaction`/`debug_traceBlock` when the reconstructed trace state can't cover the upfront gas buy; log and continue without charging the fee, and log skipped/failed predecessors. `debug_traceCall` still surfaces the `insufficient funds` error, matching go-ethereum.
