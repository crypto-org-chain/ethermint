...
// Standalone path (resBlock == nil) resolves the tx via the KV indexer in
// O(1); res.CumulativeGasUsed is the block-wide eth cumulative recorded
// at index time. When the caller passes a resBlock whose height differs
// from the indexer's (indexer-overwrite guard), the receipt is rebuilt
// from the caller-supplied block by walking its txs.
...
// assembles the receipt directly from that block's result. O(1) — does
// not walk the block.
