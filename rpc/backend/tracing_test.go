...
// TestTraceTransactionWithReplay traces a transaction with a TraceConfig that
// sets TraceReplay=true and asserts the flag is forwarded to the gRPC request.
// The assertion lives in the registerMock hook: RegisterTraceTransactionWithReplay
// only matches (and thus only returns a response) when the request carries
// TraceReplay=true, so the trace succeeds solely when the flag is propagated.
